...
- 白板长图视觉主题升级：4 套设计主题（白板讲解 / 杂志排版 / 黑板报 / 极简现代）全部重做，新增章节徽章、强调色条、呼吸装饰、首字下沉、双栏对比卡等设计元素，整体视觉密度与节奏感大幅提升。
- 白板长图新增「**AI 设计感模式**」开关（讲义长图）：勾选后允许 AI 提炼、改写、重组原文，直接产出高密度白板长图；不再保证原文一字不漏。关闭时仍走「本地切句 100% 保真」管线，行为不变。
- 新增 IPC `ai:generate-poster-doc` 与「白板长图设计师」system prompt：6 条核心原则 + 10 种 pattern 字段直写约束 + 设计自检清单，鼓励 AI 用 highlight / contrast_card / triplet_list / formula_chain 等高强调块代替大段叙述。
- 失败兜底：AI 返回 JSON 解析失败时自动回退本地全文保留布局，渲染流程不中断。

## 0.2.31 - 2026-06-29

- 四张手稿图：生成流程改为 **每张图生成完立即入画布**，不再等待 4 张全部生成完才展示；4 个槽位在第一张到达时锁定，后续图片按序填入，避免错位。
- 四张手稿图：新增每张图单独「重新生成这张」按钮，可原地替换画布上对应图片，不影响其它图片与 1/2/3/4 跳转。
- 四张手稿图：默认查看缩放调到 **158%**，第一张入画布和右下角 1/2/3/4 跳转都保持该比例，便于直接阅读。
- 四张手稿图：拆分算法避免最后一段虚胖，降低单张文字过多导致模型生成糊字的概率。
- 四张手稿图：修复批量插图时偶发破图占位，改为逐张注册文件并等待一帧后再继续。

## 0.2.26 - 2026-06-29

- 四张手稿图优化：
  - 生图提示词强制要求 **纯白 #FFFFFF 背景**（与白板画布无缝衔接），明确禁止米色纸感 / 边框 / 外框线 / 四角阴影 / 暗角等装饰，避免出图底色偏奶白、与白板背景不一致。
  - 4 张图插入尺寸改为按 **当前视口的 90% 自适应**（保持 9:16），整组在视口居中，不再写死 405×720 导致一进来就溢出可见区。
  - 「1 / 2 / 3 / 4 跳转」按钮从左侧 AI 面板迁移到 **白板右下角浮层**，关闭面板也能跳，且当前所在图高亮。

## 0.2.25 - 2026-06-29

- 合并：「AI白板长图」与「手稿绘图 + AI 辅助」合并为单一「白板长图」模块。原 AI白板长图 tab 移除，全部白板长图统一走「本地切句保真 + AI 辅助布局 + layoutV2 渲染」管线。面板从 4 tab 简化为 3 tab。
- 优化（本地布局 irToPosterV2）：
  - 修复 section 标题用整句长文（40+ 字「你会发现…」「因为分数的本质…」）当标签，导致标题换行成正文、视觉层级崩塌。长句现生成 4-8 字短标签（结语 / 落地做法 / 经典映照），原句仍进正文，保真不丢字。
  - 连续结语句合并到单一「结语」段，不再每句各开一个只有一段正文的 section（消除末尾三段「标题+一段正文」的堆砌感）。
  - 修复「道德经说"反者道之动"」「少则得，多则惑」等经典引用被吞进大段 free_paragraph 的问题：section 句现单独成 highlight 强调块。
  - 修复概览关键词被拦腰截断：「把"要我学"变成"我要学"」→「把要我学」(坏)，现取「变成」之后的结果 →「我要学」。
  - 结尾收束句「分数的本质…」现以 highlight 大字收尾，给长图一个视觉落点。

## 0.2.24 - 2026-06-29

- 修复：手稿绘图模式下错误显示「图片提示词/尺寸比例/清晰度」（image UI 泄漏到 logic mode）。
- 修复：highlight pattern 句末小红圈去除（视觉杂讯）。
- 修复：case_box label 超长时截断（避免 chip 撑爆框线）。
- 修复：scene_with_quotes 过滤只有引号/标点的空条目（避免孤儿引号独占一行）。
