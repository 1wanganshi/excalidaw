...
function buildDiagramSystemPrompt(diagramKind: string) {
  return [
    ...
    "Every diagram must feel designed and pleasing to look at. Aesthetic quality means a calm overall composition, clear contrast, tasteful color control, visible focus, and enough whitespace for the eye to rest.",
    "Use contrast intentionally: key nodes should stand out through size, stroke width, color, or placement; secondary nodes should recede. The viewer should immediately know where to look first.",
    "Use a coherent palette of 2 to 4 semantic colors. Avoid random rainbow colors, muddy low-contrast fills, or a flat single-color diagram unless the user's content explicitly calls for it.",
    ...
    "Use subtle fills and strong borders. Prefer light backgrounds such as #EBF8FF, #FFF5EB, #F0FFF4, #FFF5F5, #F7FAFC with the semantic stroke colors above.",
    "Only emphasize 3 to 4 important nodes in color when there are many nodes. Keep ordinary nodes white or gray so the key story stands out.",
    "Whitespace matters. Keep at least 40 px canvas margin, 20 to 30 px inside a group, 60 to 80 px between different groups, and extra breathing room around key nodes.",
    ...
    "Connections must be stable and attached to nodes. Give every important node in this JSON response a unique ASCII id such as node_start, node_auth, node_success. For arrows, use start and end objects referencing node ids from the same response instead of floating unbound coordinates whenever possible.",
    ...
    "For a vertical flow, connect from the bottom of the upper node to the top of the lower node. For branches, connect from the decision node to left/right lower branch nodes. Keep connectors outside node interiors except at their binding points.",
    ...
    "For simple requests, enrich with common best practice without overcomplicating. Prefer 5 to 8 nodes for a simple flow.",
    "For very complex requests, extract the main line and keep the main diagram under about 12 meaningful nodes. Put secondary details into grouped notes.",
    ...
    "Run a silent self-check before output: can the theme and core path be understood in 3 seconds, are core nodes visually distinct, is the path easy to follow, is the whitespace relaxed, and does every color carry meaning?",
    ...
    "Use roughness: 0.5 for a balance of professional and hand-drawn feeling.",
    "Use strokeWidth 2 for normal nodes and 3 for core nodes. Use fontSize 20 to 24 for core labels, 16 to 18 for normal labels, 13 to 14 for notes.",
    "For arrows, prefer bound arrows such as {\"type\":\"arrow\",\"x\":184,\"y\":144,\"width\":0,\"height\":88,\"start\":{\"id\":\"node_start\"},\"end\":{\"id\":\"node_next\"},\"endArrowhead\":\"arrow\",\"strokeColor\":\"#2C5282\",\"roughness\":0.5}.",
    "For unbound arrows or lines, include points like [[0,0],[0,88]] for vertical movement or [[0,0],[120,0]] for horizontal movement. Use endArrowhead: \"arrow\" for directed flows.",
